if (!window.injected) {
    ...
    // Override getParameter method for WebGL
    (function () {
        ...
        WebGLRenderingContext.prototype.getParameter = function (parameter) {
            ...
            if (dbgRenderInfo) {
                if (parameter === dbgRenderInfo.UNMASKED_VENDOR_WEBGL) {
                    console.log('Intercepted UNMASKED_VENDOR_WEBGL parameter. Returning custom vendor.');
                    return fakeProfile.WebGL.Vendor;
                }
                if (parameter === dbgRenderInfo.UNMASKED_RENDERER_WEBGL) {
                    console.log('Intercepted UNMASKED_RENDERER_WEBGL parameter. Returning custom renderer.');
                    return fakeProfile.WebGL.Renderer;
                }
            }

            console.log(`Accessed parameter ${parameter}. Returning original value.`);
            return originalGetParameter.call(this, parameter);
        };

        // Error handling for getContext
        const originalGetContext = HTMLCanvasElement.prototype.getContext;
        HTMLCanvasElement.prototype.getContext = function (type, ...args) {
            const context = originalGetContext.apply(this, [type, ...args]);
            if (context && (type === "webgl" || type === "webgl2")) {
                console.log("WebGL context successfully retrieved.");
            } else {
                console.log(`Failed to retrieve WebGL context of type: ${type}`);
            }
            return context;
        };
    })();

    // Override the toDataURL method
    (function () {
        const originalToDataURL = HTMLCanvasElement.prototype.toDataURL; // Keep a reference to the original method

        HTMLCanvasElement.prototype.toDataURL = function () {
            // Call the original function to get the original data URL
            let originalData = originalToDataURL.call(this); // Use the reference to the original method

            // Log the original data URL
            console.log('Original Data URL:', originalData);

            // Generate a random string
            let randomString = generateRandomString(10); // Change the length as needed

            // Append the random string to the data URL
            let modifiedData = `${originalData}?random=${randomString}`; // Append modification

            // Log the modified data URL
            console.log('Modified Data URL:', modifiedData);

            return modifiedData;
        };
    })();

    // Inject Canvas and WebGL context
    (function injectCanvas() {
        document.addEventListener("DOMContentLoaded", function () {
            ...
            if (!gl) {
                ...
                console.log("Injected Canvas and WebGL context created successfully.");
            }

            // Optional WebGL configuration
            gl.clearColor(0.0, 0.0, 0.0, 1.0); // Clear color: black, fully opaque
            gl.enable(gl.DEPTH_TEST);          // Enable depth testing
            gl.depthFunc(gl.LEQUAL);           // Set depth function
            gl.clear(gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT); // Clear color and depth buffer

            // Example: Drawing commands would go here (if shaders are set up)
        ...
        function injectMediaDevices(config) {
            ...
            var injectWebGl = function (config) {
                ...
                var customVendor = "Custom Vendor"; // Set your custom vendor here
                var customRenderer = "Custom Renderer"; // Set your custom renderer here
                var webglUtil = {
                    "random": {
                        "value": function () {
                            return Math.random()
                            ...
                            "parameter": function (target) {
                                ...
                                Object.defineProperty(target.prototype, "getParameter", {
                                    ...
                                    "value": function () {
                                        ...
                                        // Custom spoof for VENDOR and RENDERER
                                        if (webglCode === 7936) { // VENDOR
                                            return customVendor;
                                        } else if (webglCode === 7937) { // RENDERER
                                            return customRenderer;
                                        }
                                        // Original checks in webglParams, if defined
